...
as we can see from the table, the trend is not apparent.
...
acc_tidy_data %>%
  select(
    day_id,minute,activity_counts
  )  %>% 
  ggplot(aes(x = minute, y = activity_counts, color = day_id)) +   
  geom_smooth()
```
...
this dataset has  `r nrow(ny_noaa)`  rows and it contains `r names(ny_noaa)`  `r ncol(ny_noaa)` variables. andvariables
...
```
The most commonly observed is `0`, because NYC doesn't snow for most of the time.
